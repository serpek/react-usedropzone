...
const Dropzone = forwardRef(({ children, ...params }, ref) => {
  const { open, setFiles, ...props } = useDropzone(params);

  useImperativeHandle(ref, () => ({ open, setFiles }), [open, setFiles]);

  // TODO: Figure out why react-styleguidist cannot create docs if we don't return a jsx element
  return <Fragment>{children({ ...props, open, setFiles })}</Fragment>;
});
...
const defaultProps = {
  ...
  preventPasteOnDocument: true,
  noClick: false,
  noKeyboard: false,
  noDrag: false,
  noPaste: false,
  noDragEventsBubbling: false,
  noPasteEventsBubbling: false,
  validator: null,
  useFsAccessApi: true,
  autoFocus: false,
  pasteContainerRef: null,
};
...
Dropzone.propTypes = {
   ...
   * @param {Function} params.setFiles Open the native file selection dialog
  ...
  preventPasteOnDocument: PropTypes.bool,
  ...
  noPasteEventsBubbling: PropTypes.bool,
...
export function useDropzone(props = {}) {
  const {
    ...
    preventPasteOnDocument,
    noClick,
    noKeyboard,
    noDrag,
    noPaste,
    noDragEventsBubbling,
    noPasteEventsBubbling,
    onError,
    validator,
    pasteContainerRef,
  } = {
  ...
  const pasteTargetsRef = useRef(null);
  ...
  const onDocumentPaste = (event) => {
    const target =
      pasteContainerRef === null ? document : pasteContainerRef.current;
    if (rootRef.current && target.contains(event.target)) {
      console.log(event);
      // If we intercepted an event for our instance, let it propagate down to the instance's onDrop handler
      return;
    }
  ...
  useEffect(() => {
    const target =
      pasteContainerRef === null ? document : pasteContainerRef.current;
    if (preventPasteOnDocument) {
      target.addEventListener("paste", onDocumentPaste, false);
    }

    return () => {
      if (preventPasteOnDocument) {
        target.removeEventListener("paste", onDocumentPaste);
      }
    };
  }, [rootRef, pasteContainerRef, preventPasteOnDocument]);
  ...
  const setFiles = useCallback(
    (files, event) => {
      ...
      if (onPaste) {
        onPaste(acceptedFiles, fileRejections, event);
      }
  ...
  const onPasteCb = useCallback(
    (event) => {
      ...
      if (isEvtWithFiles(event)) {
        Promise.resolve(getFilesFromEvent(event))
          .then((files) => {
            if (isPropagationStopped(event) && !noPasteEventsBubbling) {
              return;
            }
            setFiles(files, event);
          })
    ...
    [getFilesFromEvent, setFiles, onErrCb, noPasteEventsBubbling]
  );
  ...
  const stopPropagation = (event) => {
    if (noDragEventsBubbling || noPasteEventsBubbling) {
  ...
  return {
    ...
    setFiles: composeHandler(setFiles),
  };
}
